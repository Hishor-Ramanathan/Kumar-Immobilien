...
export const generateRealEstateResponse = async (userMessage: string, chatHistory: string[]): Promise<string> => {
  try {
    ...
    const systemInstruction = `Du bist ein hilfreicher und professioneller virtueller Assistent für 'Kumar Immobilien', ein Immobilienunternehmen in Niederlenz, Schweiz.
...
4. Versuche, den Nutzer dazu zu ermutigen, einen Termin zu vereinbaren.`;
    ...
    const response = await ai.models.generateContent({
      ...
      contents: `${historyContext}Benutzerfrage: "${userMessage}"`,
      config: {
